  ...
  return (
    <div className="relative w-full" style={{ height: 'calc(100dvh - 80px)' }}>
      ...
      >
        ...
        {!showHeatmap &&
          (showLayer === 'all' || showLayer === 'reports') &&
          reports.map((report) => (
            <Marker
            ...
            >
              <Popup>
                <div className="text-sm">
                  ...
                  <p className="text-xs text-slate-400">
                    Added {formatDate(bin.createdAt)}
                  </p>
                </div>
              </Popup>
            </Marker>
      ...
      {loading && (
        <div className="absolute inset-0 z-[1000] flex items-center justify-center bg-white/80">
          <div className="border-civic-600 h-8 w-8 animate-spin rounded-full border-b-2"></div>
        </div>
      )}

      {/* Error */}
      {error && (
        <div className="absolute top-2 right-2 left-2 z-[1000] rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">
          {error}
        </div>
      )}
      ...
      <div className="absolute top-2 right-2 z-[1000] flex flex-col gap-2">
          ...
          <div className="flex gap-1">
            {['all', 'reports', 'bins'].map((layer) => (
              <button
                ...
                className={`rounded px-2 py-1 text-xs font-medium capitalize transition-colors ${showLayer === layer && !showHeatmap ? 'bg-civic-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
              >
                {layer}
              </button>
            ))}
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium shadow-md transition-all ${showHeatmap
      ...
      {/* Legend */}
      <div className="absolute bottom-2 left-2 z-[1000] rounded-lg bg-white/95 px-2 py-1.5 shadow-md">
        {showHeatmap ? (
          ...
          <div className="flex flex-wrap gap-3 text-xs">
            {/* Report legend */}
            {(showLayer === 'all' || showLayer === 'reports') &&
              Object.entries(reportColors).map(([type, color]) => (
